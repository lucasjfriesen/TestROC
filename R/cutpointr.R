...
cutpointr_internal <- function(x, class, subgroup, method, metric, pos_class,
    ...
    if (!is.null(subgroup)) {
        ...
        dat <- dplyr::mutate_(tidyr::nest_(dplyr::group_by_(dat, "subgroup"), key_col = "data"), subgroup = ~ as.character(subgroup))
        ...
        if (na.rm) dat <- stats::na.omit(dat)
        dat <- tidyr::nest_(dat, key_col = "data")
        dat$pos_class <- pos_class
    ...
    if (boot_runs <= 0) {
        ...
        bootstrap <- dplyr::transmute_(dat, boot = ~ purrr::map2(dat$data, dat$pos_class,
                                                    function(g, pc) {
